...
def test_native_crash_uses_conservative_first_party_fallback(monkeypatch):
    monkeypatch.setattr(
        "polaris_modernization.isolated_extraction.subprocess.run",
        lambda *args, **kwargs: subprocess.CompletedProcess(args[0], 3221225477, "", ""),
    )

    result = extract_file(request("services/alpha.js"))

    assert result.warning is None
    assert any(fact.kind == "conservative_source" for fact in result.facts)
    assert not any(fact.kind == "opaque_source" for fact in result.facts)


def test_native_crash_uses_opaque_only_for_generated_or_dependency_content(monkeypatch, tmp_path):
    source = tmp_path / "generated.designer.cs"
    source.write_text("// <auto-generated>\npublic class Generated {}", encoding="utf-8")
    monkeypatch.setattr(
        "polaris_modernization.isolated_extraction.subprocess.run",
        lambda *args, **kwargs: subprocess.CompletedProcess(args[0], 3221225477, "", ""),
    )

    result = extract_file({**request("generated.designer.cs"), "source_root": str(tmp_path), "language": "csharp"})

    assert result.warning is None
    assert [fact.kind for fact in result.facts] == ["opaque_source"]
