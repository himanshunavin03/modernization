...
def test_narrative_synthesis_groups_and_traces_all_contracts(tmp_path):
    result=synthesize_feature_narratives(SOURCE,tmp_path/"out",KG);v=result["validation"]
...
def test_customer_documents_use_eight_sections_and_preserve_json(tmp_path):
    synthesize_feature_narratives(SOURCE,tmp_path/"out",KG)
    ...
    assert "## 1. Feature Summary" in md and "## 5. Existing Backend API Contract" in md and "## 8. Review & Approval" in md
...
def test_api_contracts_are_evidence_backed_and_preserve_status(tmp_path):
    result=synthesize_feature_narratives(SOURCE,tmp_path/"out",KG)
    root=tmp_path/"out/latest"
