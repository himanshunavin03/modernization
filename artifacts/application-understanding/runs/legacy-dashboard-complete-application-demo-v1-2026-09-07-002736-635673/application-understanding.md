...
- KG run: `legacy-dashboard-complete-application-demo-v1-2026-09-06-203944`
...
- **Dashboard requests clinic summary**: Dashboard requests clinic summary through a frontend call linked to its backend endpoint.
- **Dashboard requests yearly expenses**: Dashboard requests yearly expenses through a frontend call linked to its backend endpoint.
- **Dashboard requests yearly patient report**: Dashboard requests yearly patient report through a frontend call linked to its backend endpoint.
...
- **Dashboard depends on reports API contract**: Dashboard service calls clinic-summary and year-dependent report endpoints through proven frontend-to-backend relationships.
- **Doctor UI depends on doctor API contract**: Doctor UI and service operations use linked doctor endpoints; automatic tenant context remains a separate prerequisite.
- **Patient UI depends on patient API contract**: Patient UI and service operations use linked patient endpoints; automatic tenant context remains a separate prerequisite.
