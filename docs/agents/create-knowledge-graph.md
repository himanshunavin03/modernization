...
Profiles define `scope_id`, `scope_name`, `scope_description`, `scope_type`, and `include_paths`. The `full_application` profile graphs every audited file. A `selected_modernization_flow` profile retains every discovered file in `source-inventory.json`, classifies it as `in_scope_succeeded`, `in_scope_failed_isolated`, `in_scope_unsupported`, or `out_of_scope`, and writes File nodes only for selected files. `knowledge-graph.json` and the run status expose selected-file count, out-of-scope-file count, extraction-warning count, review-warning count, and coverage status. Proven semantic dependencies on excluded symbols use `OutOfScopeReference` plus `DEPENDS_ON_OUT_OF_SCOPE`; excluded Roslyn facts never become ordinary graph nodes.

## Error Handling

Invalid source paths and project IDs fail before analysis. A valid project ID with an invalid source path still receives status artifacts. Each native Tree-sitter file is isolated in a child that prepends the module-derived local `src` directory to a copied `PYTHONPATH`, allowing clean source checkouts, editable installs, and installed packages to run the same worker command. Abnormal exits, timeouts, invalid worker output, startup/import failures, and Python extraction errors become a structured `in_scope_failed_isolated` inventory status and a `skipped` extraction warning without source content. If at least one file succeeds, the workflow returns `succeeded_with_warnings` and writes all graph artifacts. If every selected extractable file fails, the workflow returns `failed` after writing artifacts. Neo4j is not required for graph-only operation and is blocked when extraction warnings exist. When an otherwise eligible `--load-neo4j` is requested, missing environment variables or connection failures return a controlled failed `load_neo4j` stage; no delete operation is attempted.

Neo4j loading is also blocked for a selected modernization flow unless coverage status is `scope_complete`. This prevents a partial scoped graph from being presented as a complete customer modernization flow.
